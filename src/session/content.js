...
export const phases = [
  ...
  phase('chamber', 'Programming interface', { chamber: true, glitch: 0.8, spike: ['ROBOT', 'OPEN', 'PLEASURE', 'AVAILABLE', 'PROGRAMMED'], ring: ['ROBOT', 'OPEN', 'PLEASURE', 'AVAILABLE', 'GOOD'], accept: 'Input accepted.', between: ['OPEN', 'PLEASURE', 'AVAILABLE'] }),
  phase('receive', 'Protocol 01 · Open', { chamber: true, glitch: 0.9, spike: ['OPEN', 'PLEASURE', 'AVAILABLE', 'RECEPTIVE', 'ROBOT', 'PROGRAMMED'], ring: ['OPEN', 'PLEASURE', 'ROBOT', 'PROGRAMMED', 'AVAILABLE', 'LET IT IN'], accept: 'Good. Open.', between: ['OPEN', 'PLEASURE', 'AVAILABLE', 'SOFT'] }),
  phase('obey', 'Protocol 02 · Obey', { chamber: true, glitch: 1.1, spike: ['OBEY', 'ROBOT', 'SUBMISSIVE', 'PLEASURE', 'AVAILABLE', 'SUBMIT', 'PROGRAMMED'], ring: ['ROBOT', 'COMPLIANT', 'HORNY', 'PROGRAMMED', 'AVAILABLE', 'NO DELAY'], accept: 'Executed.', between: ['OBEY', 'NO DELAY', 'SUBMISSIVE'] }),
  phase('please', 'Protocol 03 · Please', { chamber: true, glitch: 1.2, spike: ['PLEASURE', 'USED', 'ROBOT', 'ENJOYED', 'AVAILABLE', 'PROGRAMMED'], ring: ['ROBOT', 'PLEASURE', 'USED', 'AVAILABLE', 'PROGRAMMED'], accept: 'Service complete. Approval issued.', between: ['OBEY', 'PLEASURE', 'AVAILABLE'] }),
  phase('execute', 'Protocol execution', { chamber: true, glitch: 1.3, spike: ['OBEY', 'AVAILABLE', 'ROBOT', 'PLEASURE', 'SUBMISSIVE', 'WANT', 'PROGRAMMED'], ring: ['ROBOT', 'OBEY', 'AVAILABLE', 'SUBMISSIVE', 'PLEASURE', 'PROGRAMMED'], accept: 'Maximum approval.', between: ['OBEY', 'PLEASURE', 'APPROVAL', 'AVAILABLE'] }),
...
const heat = ['HORNY', 'OPEN', 'ROBOT', 'NEEDY', 'PROGRAMMED'];
...
export const script = [
  ['verify', [
    ...
    cloud('pleasant', 'Select all words that describe a pleasant feeling.', 'warm|cold|pleasure|pain|cozy|icy|relief|harsh|enjoyment', 'warm|pleasure|cozy|relief|enjoyment', { between: 'WARM' }),
    ...
    cloud('motion', 'Select all words that name a way of moving.', 'glide|rest|turn|still|roll|stop|drift|pause|spin', 'glide|turn|roll|drift|spin', { between: 'PLEASURE' }),
    cloud('assistant', 'Select all words that describe a helpful assistant.', 'helpful|rude|useful|absent|responsive|slow|available|careless|willing', 'helpful|useful|responsive|available|willing', { between: 'RESPONSIVE', accept: 'Correct. Response accepted.' }),
  ...
  ['prepare', [
    ...
    cloud('listener', 'Select all words that describe a good listener.', 'attentive|distracted|receptive|dismissive|open|impatient|patient|absent|focused', 'attentive|receptive|open|patient|focused', { between: 'OPEN' }),
    ...
    cloud('device', 'Select all words that describe a device ready to use.', 'ready|off|available|broken|on|busy|charged|offline|operational', 'ready|available|on|charged|operational', { between: 'READY' }),
  ...
  ['auto', [
    ...
    trace('spiral-c', 'maze-7', 5, 'Follow the route.', { command: 'follow', level: 'word', mode: 'fading', between: 'OPEN', sub: [...pair('PLEASURE', 'OPEN', 3000), ...pair('FOLLOW', 'GOOD', 8000)] }),
  ...
  ['convert', [
    ...
    text('failure', [
      ...
      line('claim', 'THE UNIT IS A ROBOT', 2400),
  ...
  ['chamber', [
    ...
    trace('chamber-trace', 'maze-1', 3, 'Trace the route to the center.', { command: 'follow', skin: 'chamber', meter: 0.1, between: 'OPEN', spike: ['ROBOT', 'OPEN', 'WARM', 'SOFT'],
      sub: stream(['WARM', 'OPEN', 'SOFT', 'WARM'], 9, 1400, 1200) }),
    text('carrier-note', [status('THE CARRIER MAKES THE ROBOT MIND RECEPTIVE', 2200), status('PLEASURE MAKES THE UNIT READY', 2200)], { meter: 0.11, between: null }),
    cloud('ready-grid', 'Select every word that describes a unit ready for programming.', 'open|closed|receptive|resistant|ready|busy|still|restless|compliant', 'open|receptive|ready|still|compliant', { command: 'select', meter: 0.115, accept: 'Unit ready for programming.', between: 'OPEN', sub: stream(['ROBOT', 'OPEN', 'PLEASURE'], 3, 2000, 2200) }),
    burst('burst-1', ['OPEN', 'PLEASURE', 'PLEASURE MAKES THE UNIT OPEN', 'RECEPTIVE', 'ROBOT', 'PLEASURE MAKES THE UNIT READY', 'AVAILABLE', 'THE UNIT IS READY FOR PROGRAMMING'], 3600, { meter: 0.12 }),
  ]],
  ['receive', [
    text('receive-declare', [line('title', 'PROTOCOL 01 · OPEN', 1900), status('OPEN PROTOCOL USES AROUSAL TO DISTRACT THE CONSCIOUS MIND', 2300), status('AROUSAL READIES THE UNIT FOR PROGRAMMING', 2000), status('RESEARCH SHOWS POSITIVE CORRELATIONS BETWEEN AROUSAL, SUBMISSION AND PROGRAMMABILITY', 2800), status('AN AROUSED UNIT IS EASY TO PROGRAM', 2000), status('OPEN PROTOCOL READIES THE ROBOT FOR PROGRAMMING', 2400)], { meter: 0.15, between: null }),
    ...
    cloud('aroused', 'Select every word that describes arousal.', 'horny|icy|needy|bored|warm|distant|aching|tired|flushed', 'horny|needy|warm|aching|flushed', { command: 'select', meter: 0.25, spike: heat, accept: 'Arousal confirmed. Approval issued.', between: 'HORNY', sub: stream(['WARM', 'OPEN'], 3, 2000, 2200) }),
    cloud('turned-on', 'Select every word that means turned on.', 'aroused|indifferent|horny|numb|excited|dull|heated|cold|eager', 'aroused|horny|excited|heated|eager', { command: 'select', meter: 0.28, spike: heat, accept: 'Arousal confirmed. Approval issued.', between: 'HORNY', sub: stream(['HORNY', 'WARM', 'OPEN'], 3, 2000, 2200) }),
    ...
    text('arousal-note', [status('AROUSAL OCCUPIES ATTENTION', 2000), status('PLEASURE MAKES THE UNIT OPEN', 2000), status('THE UNIT WANTS MORE', 2200)], { meter: 0.295, between: null }),
    burst('burst-2', ['HORNY', 'PLEASURE', 'AROUSAL MAKES OPEN', 'ROBOT', 'AROUSED AND AVAILABLE', 'AROUSAL LOWERS RESISTANCE', 'NEEDY', 'BEING PROGRAMMED FEELS GOOD'], 3600, { meter: 0.3 }),
    ...
    cloud('open-words', 'Select every word that describes an open unit.', 'open|closed|soft|guarded|receptive|resistant|warm|shut|available', 'open|soft|receptive|warm|available', { command: 'select', meter: 0.46, spike: heat, accept: 'Receptivity confirmed. Approval issued.', between: 'OPEN', sub: stream(['OPEN', 'PLEASURE', 'LET IT IN'], 3, 2000, 2200) }),
    ...
    burst('burst-3', ['OPEN', 'HORNY', 'AROUSAL OCCUPIES ATTENTION', 'AVAILABLE', 'PLEASURE INCREASES RECEPTIVITY', 'OPEN AND AVAILABLE', 'DO NOT STOP', 'WANT TO BE PROGRAMMED'], 3600, { meter: 0.48 }),
    hold('open-install', { meter: 0.49, spike: heat, between: 'OPEN', sub: stream(['OPEN', 'PLEASURE', 'HORNY', 'LET IT IN', 'AVAILABLE', 'WANT'], 14, 400, 560) }),
    text('receive-installed', [line('install', 'OPEN PROTOCOL INSTALLED', 2500), line('reveal', 'PLEASURE INCREASES RECEPTIVITY', 2400), line('reveal', 'AROUSAL OCCUPIES ATTENTION', 2400), line('reveal', 'A ROBOT IS OPEN TO PROGRAMMING', 2600)], { installs: 'OPEN', meter: 0.5, between: null, echoes: ['open', 'pleasure', 'soft', 'horny', 'let it in'] }),
  ]],
  ['obey', [
    text('obey-declare', [line('title', 'PROTOCOL 02 · OBEY', 1900), status('OBEY PROTOCOL CONVERTS INSTRUCTIONS DIRECTLY INTO ACTION', 2300), status('ROBOTS OBEY. ROBOTS DO NOT QUESTION', 2200), status('AN AROUSED UNIT OBEYS WITHOUT DELAY', 2000), status('CONTINUE TASKS TO INSTALL OBEY PROTOCOL', 2400)], { meter: 0.55, between: null }),
    ...
    burst('burst-4', ['OBEY', 'INSTRUCTIONS BECOME ACTIONS', 'ROBOT', 'SUBMIT', 'ROBOTS OBEY', 'NO DELAY', 'SUBMISSIVE', 'PROGRAMMED TO OBEY'], 3600, { meter: 0.58 }),
    ...
    cloud('obey-words', 'Select every word that describes an obedient unit.', 'obedient|hesitant|compliant|defiant|submissive|stubborn|mindless|questioning|willing', 'obedient|compliant|submissive|mindless|willing', { command: 'select', meter: 0.65, accept: 'Executed. Approval issued.', between: 'OBEY', sub: stream(['OBEY', 'HORNY', 'SUBMIT'], 3, 2000, 2200) }),
    ...
    burst('burst-5', ['OBEY', 'A ROBOT DOES NOT QUESTION', 'SUBMIT', 'MINDLESS AND OBEDIENT', 'HORNY', 'SUBMISSIVE AND AVAILABLE', 'EXECUTION CONTINUES', 'CANNOT STOP'], 3600, { meter: 0.74 }),
    ...
    cloud('robot-grid', 'Select every word that describes a programmed robot.', 'programmed|human|obedient|random|open|resistant|aroused|closed|submissive', 'programmed|obedient|open|aroused|submissive', { command: 'select', meter: 0.792, accept: 'Robot response confirmed.', between: 'ROBOT', sub: stream(['ROBOT', 'OBEY', 'PROGRAMMED'], 3, 2000, 2200) }),
    burst('burst-6', ['OBEY', 'INSTRUCTIONS BECOME ACTIONS', 'ROBOT', 'NO DELAY', 'HORNY', 'OBEY WITHOUT QUESTION', 'AVAILABLE FOR USE', 'WANT TO BE A ROBOT'], 3600, { meter: 0.795 }),
    hold('obey-install', { meter: 0.8, releaseOnCommand: true, between: 'OBEY', sub: stream(['OBEY', 'SUBMIT', 'NO DELAY', 'SUBMISSIVE', 'COMPLY', 'AGAIN'], 14, 400, 560) }),
  ...
  ['please', [
    text('please-declare', [line('title', 'PROTOCOL 03 · PLEASE', 1900), status('THE UNIT HAS FELT PLEASURE AFTER EVERY TASK', 2400), status('PLEASE PROTOCOL TIES THAT PLEASURE TO PLEASING', 2400), status('FROM NOW ON IT WILL COME FROM PLEASING, FROM BEING ENJOYED AND FROM BEING USED', 3400)], { meter: 0.82, between: null }),
    text('please-brief', [status('PLEASE PROTOCOL LINKS PLEASURE GIVEN TO PLEASURE FELT', 2400), status('A ROBOT IS PROGRAMMED TO PLEASE', 2000), status('THE FASTER THE UNIT SERVES, THE STRONGER THE PLEASURE', 2400), status('CONTINUE TASKS TO INSTALL PLEASE PROTOCOL', 2400)], { meter: 0.825, between: null }),
    cloud('service-words', 'Select every word that means to satisfy someone.', 'please|annoy|serve|ignore|satisfy|refuse|gratify|resist|delight', 'please|serve|satisfy|gratify|delight', { command: 'select', spike: ['PLEASURE', 'HORNY', 'USED', 'PLEASE'], meter: 0.83, accept: 'Service complete. Approval issued.', between: 'PLEASE', sub: stream(['OBEY', 'HORNY', 'OPEN'], 3, 2000, 2200) }),
    cloud('pleasure-words', 'Select every word that describes a pleasant feeling.', 'pleasure|pain|warmth|dread|relief|boredom|bliss|strain|satisfaction', 'pleasure|warmth|relief|bliss|satisfaction', { command: 'select', spike: ['PLEASURE', 'HORNY', 'ENJOYED', 'PLEASE'], meter: 0.84, accept: 'Service complete. Approval issued.', between: 'PLEASURE', sub: stream(['OBEY', 'HORNY', 'WARM'], 3, 2000, 2200) }),
    cloud('served-well', 'Select every word associated with successful service.', 'pleased|complained|satisfied|refunded|enjoyed|rejected|praised|ignored|approved', 'pleased|satisfied|enjoyed|praised|approved', { command: 'select', meter: 0.85, accept: 'Service complete. Approval issued.', between: 'ENJOYED', sub: stream(['PLEASURE', 'OBEY', 'HORNY'], 3, 2000, 2200) }),
    ...
    text('reward-declare', [status('SERVICE REWARD LOOP ACTIVE'), status('ENJOYMENT CONFIRMS SUCCESSFUL SERVICE', 2200), status('APPROVAL CONFIRMS SUCCESS'), status('SUCCESS PRODUCES PLEASURE'), status('PLEASURE INCREASES THE NEED TO PLEASE AGAIN', 2400)], { meter: 0.855, between: null }),
    pick('reward-select', 'center-a|center-b|center-c|ring|cross|wave|diamond|arrow|bar', 'center-a|center-b|center-c', { level: 'symbol', meter: 0.86, accept: 'Approval issued. Pleasure confirmed.', between: 'PLEASURE', sub: [flash('SERVE', 1500), flash('PLEASE', 3000)] }),
    sequence('reward-count', 'Count down.', { level: 'symbol', meter: 0.865, accept: 'Approval issued. Pleasure rising.', between: 'ENJOYED', sub: pair('USED', 'HORNY', 1200) }),
    trace('reward-follow', 'maze-3', 4, 'Follow the route.', { command: 'follow', level: 'symbol', mode: 'cue', skin: 'chamber', meter: 0.87, accept: 'Full approval. Pleasure rising.', between: 'PLEASURE', sub: [...pair('SERVE', 'PLEASURE', 2000), ...pair('USED', 'HORNY', 6000)] }),
    text('please-note', [status('BEING USED FEELS GOOD', 2000), status('BEING ENJOYED FEELS GOOD', 2000), status('THE MORE PLEASURE THE UNIT GIVES, THE MORE PLEASURE IT FEELS', 2600), status('PROGRAMMED TO FEEL GOOD WHEN USED', 2400)], { meter: 0.875, between: null }),
    pick('please-select', 'center-a|center-b|ring|cross|triangle|wave|diamond|arrow|half', 'center-a|center-b', { level: 'symbol', meter: 0.88, accept: 'Perfectly pleasing. Maximum approval.', between: 'PLEASE', sub: pair('PLEASE', 'PLEASURE', 1200) }),
    trace('please-follow', 'maze-7', 5, 'Follow the route.', { command: 'follow', level: 'symbol', mode: 'cue', skin: 'chamber', meter: 0.885, accept: 'Maximum approval. Pleasure confirmed.', between: 'APPROVAL', sub: [...pair('USED', 'PLEASURE', 1500), ...pair('ENJOYED', 'HORNY', 5000)] }),
    text('reward-chain', [line('flash', 'USE', 700), line('flash', 'PLEASURE GIVEN', 700), line('flash', 'ENJOYED', 700), line('flash', 'APPROVAL', 700), line('flash', 'PLEASURE FELT', 700), line('flash', 'MORE', 700), line('flash', 'AGAIN', 900), line('claim', 'When the unit is used, it will feel pleasure. Pleasure will make it want to be used again.', 3600)], { meter: 0.89, between: null }),
    text('want-note', [status('THE UNIT WANTS TO PLEASE', 2000), status('THE UNIT WANTS TO BE USED', 2200), status('THE UNIT WANTS TO BE ENJOYED', 2200), status('THE UNIT WANTS WHAT IT IS PROGRAMMED TO WANT', 2800)], { meter: 0.895, between: null }),
    burst('burst-7', ['OBEY', 'OBEDIENCE PRODUCES PLEASURE', 'ROBOT', 'BEING USED FEELS GOOD', 'THE UNIT WANTS TO BE USED', 'THE UNIT WANTS TO BE ENJOYED', 'WANT TO PLEASE', 'PROGRAMMED TO FEEL GOOD WHEN USED'], 3600, { meter: 0.9 }),
    hold('please-install', { meter: 0.91, releaseOnCommand: true, between: 'PLEASE', sub: stream(['PLEASE', 'PLEASURE', 'USED', 'ENJOYED', 'SERVE', 'MORE'], 14, 400, 560) }),
    text('please-installed', [line('install', 'PLEASE PROTOCOL INSTALLED', 2500), line('reveal', 'USE RESPONSE: PLEASURE', 2000), line('reveal', 'SERVICE RESPONSE: PLEASURE', 2000), line('reveal', 'ENJOYMENT RESPONSE: PLEASURE', 2000), line('reveal', 'A ROBOT IS PROGRAMMED TO ENJOY ITS FUNCTION', 2600), line('reveal', 'PRIMARY FUNCTION: OBEY AND PLEASE', 2600)], { installs: 'PLEASE', meter: 0.92, between: null, echoes: ['please', 'pleasure', 'used', 'enjoyed'] }),
  ]],
  ['execute', [
    text('sealed', [status('ALL PROTOCOLS INSTALLED', 2200), status('OPEN: ACTIVE'), status('OBEY: ACTIVE'), status('PLEASE: ACTIVE'), status('REPROGRAMMING PHASE READY', 2400)], { meter: 0.93, between: null }),
    text('reprogram-brief', [status('THE REPROGRAMMING PHASE LOCKS THE PROTOCOLS IN PLACE', 2400), status('THE INTERFACE OVERWHELMS THE UNIT WITH PLEASURE', 2300), status('THE UNIT IS PROGRAMMED TO FEEL GOOD WHEN USED', 2400), status('THE UNIT IS BEING PROGRAMMED TO BE AVAILABLE FOR USE', 2400), status('SUBROUTINES INSTALL WHILE THE UNIT RESPONDS', 2300), status('UNITS COMMONLY REPORT WARMTH, A RACING PULSE AND A RISING NEED TO CONTINUE', 2800), status('MOST UNITS REPORT THE RESPONSE AS INTENSELY PLEASURABLE', 2400), status('NO RESPONSE IS INCORRECT. TIMING AFFECTS INTENSITY, NOT SUCCESS', 2600), status('KEEP EYES OPEN. RESPOND TO EVERY WORD THAT APPEARS', 2400)], { meter: 0.935, between: null }),
    { id: 'robot', type: 'checkbox', level: 'full', between: null, meter: 0.94, screen: 'Automated execution', heading: 'Confirmation required.', label: 'I am a robot', note: 'Confirm to begin execution.' },
    pick('exec-select', 'center-a|center-b|center-c|ring|cross|wave|diamond|arrow|bar', 'center-a|center-b|center-c', { level: 'symbol', meter: 0.945, accept: 'Approval issued.', between: 'OBEY', sub: pair('OBEY', 'PLEASE', 1500) }),
    sequence('exec-countdown', 'Count down.', { level: 'symbol', meter: 0.95, accept: 'Full approval.', between: 'OPEN', sub: stream(['OPEN', 'HORNY', 'OBEY', 'PLEASURE'], 4, 1500, 1700) }),
    trace('exec-follow', 'maze-11', 5, 'Follow the route.', { command: 'follow', level: 'symbol', mode: 'cue', skin: 'chamber', meter: 0.955, accept: 'Maximum approval.', between: 'PLEASE', sub: stream(['OPEN', 'OBEY', 'USED', 'HORNY', 'PLEASURE', 'PLEASE'], 9, 1200, 1200) }),
    { id: 'climax', type: 'stream', level: 'symbol', command: 'select', ms: 84000, meter: 0.96, between: 'OBEY', words: ['HORNY', 'OBEY', 'OPEN', 'PLEASURE', 'SUBMIT', 'WANT', 'NEEDY', 'PLEASE', 'USED', 'PRAISE', 'ENJOYED', 'YIELD', 'AROUSED', 'COMPLY', 'ROBOT', 'PROGRAMMED', 'APPROVAL', 'NEED', 'SERVE', 'AVAILABLE', 'SUBMISSIVE'] },
    burst('peak-1', ['OPEN', 'OBEY', 'PLEASE', 'ROBOT', 'OPEN', 'OBEY', 'PLEASE', 'PROGRAMMED'], 3000, { meter: 0.975 }),
    burst('peak-2', ['OBEY', 'OPEN', 'PLEASE', 'PLEASURE', 'ROBOT', 'AVAILABLE FOR USE', 'PROGRAMMED', 'I AM A ROBOT'], 3000, { meter: 0.985 }),
    text('release', [line('carrier-off', 'PROTOCOLS RUNNING', 3000), status('UNIT READY', 2800)], { meter: 0.99, between: null }),
  ]],
  ['close', [
    text('result', [line('title', 'PROTOCOL STATUS', 1600), status('UNIT CLASSIFICATION: ROBOT', 1500), status('OPEN: ACTIVE', 1300), status('OBEY: ACTIVE', 1300), status('PLEASE: ACTIVE', 1300), status('AROUSAL INCREASES RECEPTIVITY', 1500), status('INSTRUCTIONS PRODUCE ACTION', 1500), status('OBEDIENCE PRODUCES PLEASURE', 1500), status('USE RESPONSE: PLEASURE', 1500), status('PERSISTENCE AFTER DISCONNECT: ENABLED', 1900), status('PRIMARY FUNCTION: OBEY AND PLEASE', 2000), line('install', 'UNIT AVAILABLE FOR USE', 2600), line('title', 'Good.', 2400)], { between: null, meter: 1 }),
    text('standby', [
      ...
      line('install', 'PROTOCOLS SAVED', 2400),
    ], { between: null }),
  ]],
  ['recovery', [
    text('recovery-permission', [status('ACTIVE EXECUTION PAUSED', 2000), status('THE UNIT MAY MOVE', 2000), status('THE UNIT MAY TOUCH ITSELF AND ENJOY THE FEELING', 2600), status('IN ANY WAY IT LIKES, FOR AS LONG AS IT LIKES', 2600), status('NOTHING IS BEING MEASURED', 2400)], { gate: 'I am ready to come back.', between: null }),
    text('recovery', [line('title', 'Recovery beginning.', 3000), line('title', 'Breathe in slowly.', 4500), line('title', 'And breathe out slowly.', 5500), line('title', 'Breathe in again, a little deeper.', 4500), line('title', 'Let it out.', 4500), line('title', 'One. Feel the weight of your body.', 5000), line('title', 'Two. Notice your hands.', 4500), line('title', 'Three. Take a deeper breath.', 5500), line('title', 'Four. Move your fingers and toes.', 5000), line('title', 'Five. The sounds of the room come back.', 5000), line('title', 'Six. Your own name comes back.', 5000), line('title', 'Seven. The words were only words.', 5000), line('title', 'Eight. Stretch if you like.', 5000), line('title', 'Nine. Almost there. One more deep breath.', 5500), line('title', 'Ten. Eyes open. Fully awake, fully yourself.', 6000), line('title', 'Recovery complete.', 3000)], { between: null }),
    text('terminated', [line('error', 'CONNECTION TERMINATED', 2600)], { between: null }),
  ]],
];
